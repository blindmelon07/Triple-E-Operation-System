...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-header .period  { font-size: 10px; font-weight: 700; margin-top: 2px; }
        .page-header .sub     { font-size: 8px; margin-top: 1px; }
        ...
        .col-header { font-size: 9px; font-weight: 700; text-align: center; text-decoration: underline; font-style: italic; margin-bottom: 5px; }

        /* Customer block */
        .cust-name { font-size: 8px; font-weight: 700; border-bottom: 1px solid #999; padding-bottom: 1px; margin-bottom: 2px; }
        .cust-date { font-size: 6.5px; color: #555; margin-bottom: 1px; }
        ...
        .exp-row .e-desc { width: 50%; }
        .exp-row .e-date { width: 22%; font-size: 7px; color: #555; }
        .exp-row .e-amt  { width: 28%; text-align: right; }
...
<div class="page-header">
    <div class="title">Tri-E Ent. OPC Transaction Report</div>
    <div class="period">{{ $periodLabel }}</div>
    <div class="sub">
        Sessions: {{ $sessions->count() }}
        &nbsp;|&nbsp; Total Sales: &#8369;{{ number_format($totalSales, 2) }}
        &nbsp;|&nbsp; Total Expenses: &#8369;{{ number_format($totalExpenses, 2) }}
        &nbsp;|&nbsp; Cash on Hand: &#8369;{{ number_format($actualCashOnHand, 2) }}
    </div>
</div>
...
<tr>
    ...
    <td style="width:30%;">
        ...
        @forelse($namedSales as $sale)
            ...
            <div class="cust-date">{{ $sale->created_at?->setTimezone('Asia/Manila')->format('M d, Y') }}</div>
        ...
        <div class="sec-total">
            <table>
                <tr><td class="lbl">TOTAL HARDWARE SALES</td><td class="amt">{{ number_format($totalSales,2) }}</td></tr>
                @if($totalUnpaidSales > 0)
                <tr><td class="lbl unpaid">TOTAL UNPAID HARDWARE SALES</td><td class="amt unpaid">{{ number_format($totalUnpaidSales,2) }}</td></tr>
                @endif
            </table>
        </div>
    </td>

    {{-- COL 2: WALK-IN SALES + SESSIONS SUMMARY (23%) --}}
    <td style="width:23%;">
        <div class="col-header">Hardware Sales</div>
        ...
        @forelse($walkinSales as $sale)
            <div class="cust-name" style="font-style:italic;">
                Walk-in
                <span style="font-weight:400;font-size:6.5px;">&nbsp;{{ $sale->created_at?->setTimezone('Asia/Manila')->format('M d') }}</span>
            </div>
            <table class="item-tbl">
                @foreach($sale->sale_items as $item)
                    <tr>
        ...
        <div style="margin-top:8px;">
            <div style="font-weight:700;font-size:7.5px;text-decoration:underline;margin-bottom:2px;">SESSIONS SUMMARY</div>
            <table class="sess-tbl">
                <thead>
                    <tr><th>Date</th><th>Cashier</th><th class="r">Sales</th><th class="r">Cash</th></tr>
                </thead>
                <tbody>
                    @foreach($sessions as $sess)
                    <tr>
                        <td>{{ $sess->opened_at?->format('M d') }}</td>
                        <td>{{ $sess->user?->name }}</td>
                        <td class="r">{{ number_format($sess->total_sales,2) }}</td>
                        <td class="r">{{ number_format($sess->closing_amount,2) }}</td>
                    </tr>
                    @endforeach
                    <tr class="sum-row">
                        <td colspan="2">TOTAL</td>
                        <td class="r">{{ number_format($sessions->sum('total_sales'),2) }}</td>
                        <td class="r">{{ number_format($sessions->sum('closing_amount'),2) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <td style="width:20%;">
        ...
        @forelse($expenses as $expense)
            <table class="exp-row">
                <tr>
                    ...
                    <td class="e-date">{{ $expense->expense_date?->format('M d') }}</td>
        ...
        <div style="margin-top:8px;">
            ...
            @if($gcash > 0)<table class="exp-row"><tr><td class="e-desc">GCash</td><td class="e-date"></td><td class="e-amt">{{ number_format($gcash,2) }}</td></tr></table>@endif
            @if($card > 0)<table class="exp-row"><tr><td class="e-desc">Card</td><td class="e-date"></td><td class="e-amt">{{ number_format($card,2) }}</td></tr></table>@endif
            @if($paymaya > 0)<table class="exp-row"><tr><td class="e-desc">PayMaya</td><td class="e-date"></td><td class="e-amt">{{ number_format($paymaya,2) }}</td></tr></table>@endif
            @if($cod > 0)<table class="exp-row"><tr><td class="e-desc">COD</td><td class="e-date"></td><td class="e-amt">{{ number_format($cod,2) }}</td></tr></table>@endif
            <div class="div-line"></div>
            <table style="width:100%;border-collapse:collapse;">
                <tr>
    ...
    <td style="width:27%;">
        ...
        <table class="bd-tbl">
            ...
            @endforeach
            <tr class="bd-sum">
                <td colspan="2" style="font-weight:700;">REMITTED:</td>
                <td colspan="2">&nbsp;</td>
            </tr>
            <tr class="bd-sum">
                <td colspan="2" style="font-weight:700;">TOTAL CASH on HAND</td>
                <td colspan="2" style="text-align:right;font-weight:700;">{{ number_format($actualCashOnHand,2) }}</td>
            </tr>
        </table>
    </td>
...
<div class="page-footer">
    ...
    &nbsp;|&nbsp; Tri-E Enterprises OPC – Transaction Report for {{ $periodLabel }}
</div>
